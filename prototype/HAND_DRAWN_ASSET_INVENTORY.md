...
- **Status:** exploratory asset kit for a reversible prototype. Cecelia's current 5 September brand board now governs colour and type; the individual icons remain swappable and their inclusion is not user validation.
...
- **Current source cut:** 39 transcript captures, the conceptual spec, supplied product wireframes, and Cecelia's current Gaegu/`warm & fuzzies` board: deep ink `#081F4D`, steel blue `#5B7A85`, rust `#B56D5F`, ochre `#EDA343`, olive `#9F9D81`, and pale cream `#F9F9D8`.
...
| `Mail.02.png` | `cecilia/envelope-mail-02.png` | `3403c00a…040524` | Closed outlined envelope | Small courier payload or cabinet silhouette only. Do not nest it inside the full-size envelope-design or preview surface. |
...
| Envelope-design stage | The full canvas is the envelope, with edge-to-edge flap seams; `scroll-message.png` remains only an alternate template candidate. | Never place a closed-envelope icon inside the envelope. A maker-drawn seal is optional and must not gate carrier selection. |
