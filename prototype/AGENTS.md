...
- Every product state is a fixed phone composition with no vertical page scrolling, scrollbar, or rubber-band gesture. The simulated keyboard may shrink the usable canvas, but it must not create scroll. Long cabinets page through four objects at a time with explicit controls; the received paper remains full-screen and its keep/close/remove choices open temporarily from a compact bottom action instead of living below the fold.
