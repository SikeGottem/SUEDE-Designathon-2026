...
- Current interaction anchor: Home is deliberately minimal: no central decorative squiggle, with its core invitation/action bottom-centred. Begin on white with a small icon-led set of bottle, ladybug, paper-plane, and flower carriers; let the selected carrier sit alone in generous negative space (no decorative backdrop or mascot/helper copy); let each carrier change the receiver's opening gesture; build the inside as one mobile paper-sheet workspace with a compact scrollable materials rail; remove conventional bottom navigation; and let opening cross a deliberate white-to-deep-navy threshold. These are reversible prototype decisions grounded in the latest transcript and team wireframes, not validation claims.
