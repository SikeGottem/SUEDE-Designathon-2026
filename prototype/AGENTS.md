...
- Current visual anchor: the supplied `uglyhandwriting.ttf`, sparse black linework, irregular human marks, generous paper space, and restrained muted book-cover colours based on Ethan's three references. Borrow the visual grammar, not the `grug` brand, mascot, copy, or exact compositions. Give each object/state one dominant colour plus at most one support colour.
