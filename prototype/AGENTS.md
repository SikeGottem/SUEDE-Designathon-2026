...
- Bake transcript ideas into behaviour, copy, test branches, and swappable architecture according to their evidence status. Do not expose every brainstorm as a feature: locked decisions become behaviour, unresolved ideas stay reversible, and rejected ideas stay out.
...
- Cecelia owns the eventual icon and illustration language. Code-level library icons are temporary stand-ins and must remain easy to replace.
- Current visual anchor: locally bundled Gaegu Regular is the working Ugly Dave-like typeface; the supplied `uglyhandwriting.ttf` remains a fallback. Pair it with restrained, team-drawn navy linework whose apparent stroke weight sits close to Gaegu's own weight—not thick fake-sketch outlines—plus generous paper space. This font choice is provisional and must remain easy to swap. Borrow the visual grammar, not the `grug` brand, mascot, copy, or exact compositions. Give each object/state one dominant colour plus at most one support colour.
- Current interaction anchor: Home is deliberately minimal: no central decorative squiggle, with its core invitation/action bottom-centred. Begin on white with a small icon-led set of bottle, ladybug, paper-plane, and flower carriers; let the selected carrier sit alone in generous negative space (no decorative backdrop or mascot/helper copy); let each carrier change the handoff, arrival, and opening choreography. Creation is now a full-screen, capture-first Story-like mode: camera/photo/video is immediately available, successful capture drops straight into one sparse paper canvas, words and selected materials move and rotate directly, the sparse bottom tool rail stays available, prompts and selected-layer controls appear contextually, and the app chrome recedes. This interaction model supersedes the bordered paper form and below-page material rail; it borrows Story creation speed, not Instagram branding or social mechanics. Remove conventional bottom navigation and let opening cross a deliberate white-to-deep-navy threshold. These are reversible prototype decisions grounded in the newest transcript and Ethan's direct clarification, not validation claims.
- Motion is permitted only when it supplies feedback, establishes spatial consistency, shows a state change, or explains an interaction. Preserve the opening and keep/cabinet transitions; do not add ambient loops, decorative movement, or a 3D/360° collection tree.
- Cecelia's redrawn SVG assets replace provisional code geometry before final visual claims. Keep every slot independently swappable and use the asset inventory as the current typography/brush/line handoff.
