...
- Current visual anchor: locally bundled Gaegu Regular is the working Ugly Dave-like typeface; the supplied `uglyhandwriting.ttf` remains a fallback. Pair it with restrained, team-drawn navy linework whose apparent stroke weight sits close to Gaegu's own weight—not thick fake-sketch outlines—plus generous paper space. This font choice, the exact maker palette, and all team-drawn masters remain provisional and swappable. Borrow the visual grammar, not the `grug` brand, mascot, copy, or exact compositions. Keep the outer shell restrained; the made object may use a small exploratory colour system.
- Current interaction anchor: Home follows Ethan's simplified Figma composition: one small team-drawn firefly, the `warm & fuzzies` title, the invitation, and exactly one underlined primary action in generous white space. No receiver-demo, cabinet, helper, or other secondary action appears on that landing state. The maker composes the message/object first on a full-screen, non-scrolling, paper-first Story-like canvas: choose one small paper character, write directly, add/arrange optional materials, colour, hand-drawn marks and real doodles, then finish. Typed words appear as handwriting; Backspace strikes existing words instead of deleting them, paired with an unobtrusive keyboard-accessible undo/restore route. On finish, the paper visibly folds and the maker enters an envelope stage: choose from a deliberately small envelope set, decorate the outside, draw/apply a custom personal seal, then choose bottle, firefly, or paper plane as its delivery method. Envelope decoration must carry as much authored weight as carrier choice, without becoming a template/package marketplace. Carrier selection sits alone in generous negative space (no decorative backdrop or mascot/helper copy) and changes only the scoped transfer treatment. Preview shows the sealed exterior only. A one-time firefly courier may collect the selected carrier at handoff, fly away, then carry and drop it centrally at arrival; it is never a persistent mascot. The link/QR route is a real coded demo path but must never overclaim service-level privacy, delivery, retention, or size limits. Double-tap may test the opening ritual only alongside an equally direct visible tap, keyboard, and reduced-motion fallback; carrier-specific opening rituals are parked. Opening must visibly unfold the exact composed content itself: do not show a blank fold as a separate transition before a later reveal. The receiver's multi-object cabinet persists within the prototype, while product storage/deletion semantics remain honestly open; receiver-led media reveal is parked and media never auto-plays. Stickers must be team-drawn, swappable, and purposeful, never emoji or a generic sticker pack. Do not add a faux paper texture or literal Instagram camera chrome. This interaction model supersedes the bordered paper form/below-page rail, prior capture-first interpretation, carrier-before-composition order, and the flower/ladybug carrier; it borrows Story ease and direct manipulation, not Instagram branding or social mechanics. Remove conventional bottom navigation and let opening cross a deliberate white-to-deep-navy threshold. These are reversible prototype decisions grounded in the newest transcript and Ethan's direct clarification, not validation claims.
