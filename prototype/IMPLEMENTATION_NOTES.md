...
| Fold, envelope and seal | The exact authored composition visibly folds. The maker chooses one of three bounded envelope structures, may open a focused seal editor to draw/undo/clear a personal mark, and can return to the paper. The envelope surface itself carries the flap geometry; it never contains a second envelope icon. | The seal is expressive but optional and never blocks `choose how it travels`. The editor restores focus, closes with Escape, and never implies rarity or status. |
...
- **Before opening:** literal white field, deep ink linework, generous empty space, one focal object, and no conventional bottom navigation. Cecelia's current brand board supplies steel blue `#5B7A85` for the large wordmark/firefly and restrained rust `#B56D5F`, ochre `#EDA343`, olive `#9F9D81`, and pale cream `#F9F9D8` material accents.
