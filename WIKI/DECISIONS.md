...
## 2026-09-06 - Allow a bounded hosted-media handoff for normal keepsakes

- Status: current
- Decision: allow any maker to publish one immutable, exact normal keepsake with up to four photos, voice and song to an unlisted 192-bit receiver link/QR. Preserve the existing paper, Cecelia/Gaegu envelope, courier, opening and cabinet. Keep video local-only; retain the local-safe normal text path and unchanged `/demo/create` and `/demo/receive` fixture behaviour.
...
- Consequence: supported media can travel with one exact receiver artifact. Publishing is public without a code; manual cleanup alone requires an operator credential. Finalize retains its draft owner token and receiver access is link possession. The published record is immutable; a cabinet removal deletes only the browser-local reference. Limits are four 8 MiB photos, a 12 MiB voice, a 20 MiB song and 32 MiB total; 10 starts/hour, 10 pending, 100 published; one-hour pending expiry, five-minute-capped uploads and ten-minute reads.
...
| --- | --- | --- |
| Assumption and inversion | Must richer sharing mean accounts and permanent social infrastructure? | No: maker preparation plus an unlisted receiver capability keeps the flow bounded. | Whether even this infrastructure is necessary after user testing. |
| Behaviour and context | Does it still fit a private appreciation handoff? | The named receiver, envelope, courier and deliberate opening remain intact. | Sender/receiver interpretation has not been tested. |
| Medium necessity | Why not use a local link only? | Local blob media cannot cross devices; real media transfer is the tested technical need. | A non-digital or simpler exchange may better answer the brief. |
| Alternatives | Could a physical gift, direct share or public cloud link work? | Those remain valid comparators; this path preserves one composed object and ritual. | No comparative participant evidence. |
| Human specificity | Who is burdened by public quotas and link possession? | A maker explicitly prepares one gift; a receiver needs no account. | Forwarded links may affect trust; anonymous visitors can exhaust shared quotas. |
...
| Simplicity and retellability | Can the value be explained simply? | “Make one keepsake, give its exact link, open it later” remains the core story. | Shared limits need rehearsal so they do not interrupt the pitch. |
...
- Work permitted by this verdict: use the bounded public publishing flow for technical rehearsal and continue physical-device and participant checks.
...
- Main trade-off: real cross-device media improves demo truth while adding shared-quota exposure and possession-link privacy risk.
...
- Recommendation: keep public publishing bounded, describe its unlisted-link boundary honestly, and test whether the richer handoff helps the intended social moment.
...
- Public-upload correction — 6 September: Ethan explicitly requested removing the presenter-code gate and approved any visitor consuming the existing shared upload/storage allowance. This supersedes the earlier presenter-only rule. The seven judging and twelve challenge lenses above remain applicable; lower maker friction strengthens the existing solution, while public quota exhaustion is the additional feasibility/recovery risk. Cleanup and draft ownership remain guarded. The backend focused tests passed before Ethan asked to stop testing; the code-free production release `7533a6a` is now deployed at https://warm-and-fuzzies.vercel.app, and Ethan will perform the live check. No new user-validation claim is made.
...
- Evidence: [91bad8…](TRANSCRIPTS/2026-09-05-91bad8dd-1e89-586f-947f-9624f46d1fde.md) at 00:02, [7a7a60…](TRANSCRIPTS/2026-09-05-7a7a60e4-e77d-5f46-9123-82e767184889.md) at 00:01, [a39841…](TRANSCRIPTS/2026-09-05-a398410c-69a5-5f37-8b5b-0b86fc27f9d1.md) at 11:56, [b095b6…](TRANSCRIPTS/2026-09-05-b095b6db-6397-5b7c-bc97-579875233111.md) at 00:00, the transcript-derived [presentation findings](../deck/TRANSCRIPT_PRESENTATION_FINDINGS.md), and [Psychology evidence for the Warm & Fuzzies pitch](RESEARCH/FRIENDSHIP_APPRECIATION/PSYCHOLOGY_PITCH/report-source.md). The papers support bounded mechanisms only; the 1–5 ratings, positions, dot sizes, 82% and 71% are invented rehearsal values disclosed in the slide footers.
...
- Specific source anchors: the team-selected five-channel map; the hand-drawn Gaegu and navy/green palette; Ethan's point-by-point narration; the report's counterclaim that text can work.
...
- Render inspection: all six map builds, the enlarged full-height How table before and after its circle pass, and the final If / And / Then / Therefore build were checked at 1600 × 900 with no visible overflow or active-annotation collision.
...
## 2026-09-05 - Rebalance the 11-slide pitch around visible reframing, a market gap, one illustrative use case and real product mechanics

- Status: current rehearsal-deck direction, selected directly by Ethan; this refines rather than reverses the existing research and synthesis decisions.
- Decision: keep the live deck at eleven slides and a planned **5:15** runtime. Merge the former problem and occasion slides into one staged page 3 so the team's starting hypothesis visibly changes into the ordinary-day reframe and How Might We. Page 3 now moves through five beats: the starting hypothesis; highlighted `show it`; the headline `The unwritten rule: appreciation needs an occasion.` with an event side (`celebration / milestone`; expected; socially understood; a clear prompt); an ordinary-day side (`no obvious prompt`; awkward; too much; intense); and the unchanged `How might we make ordinary-day appreciation easier to send?` Keep the research findings and If / And / Then / Therefore synthesis as separate beats, and keep the solution proposition and expressive-digital value as their own beat. The former pages 7–10 move to pages 8–11 without changing their narrative jobs.
- Audience boundary: retain the broad working contexts of university students, long-distance family and close friends. Page 6 uses exactly three builds: the text constraint, the letter-or-gift constraint, then a curved pale-blue market-gap band. Its final reveal retains only `Users are stuck in limbo.` with no visible `MARKET GAP` label and no supporting explanatory sentence. This is a working need-state hypothesis, not validated market sizing, demand or prevalence. Page 7 is a separate three-build illustrative use case and begins exactly: `Maya wants to thank someone for being so friendly during her first week at uni.` It remains an ordinary Tuesday; temporary rehearsal placeholders say `72%` found a text too casual for meaningful appreciation and `78%` never received a gift just because; the slide closes that Maya is one example caught between both. Maya is not a validated persona, participant or segment.
- Product differentiation: page 10 abandons the Venn for four literal dot points grouped by one hand-drawn brace into a firefly-branded digital gift: **the personality of a handwritten letter**; **the dynamism of mixed media** through writing, photo, voice, video and song; **the ease of a text** through one direct link and no receiver account; and **the sense of ownership from a gift**, operationalised as the receiver deciding whether to keep, revisit or remove it. The live demonstration, not this slide, proves the detailed maker-to-receiver flow.
- Handoff: page 11 uses `https://warm-and-fuzzies.vercel.app/demo/receive` as the visible audience URL and QR destination; `/demo/create` remains the presenter route.
- Reason: the merge makes process visible without spending a separate slide on it; separating the market-gap claim from Maya prevents one example from being mistaken for the market; and the product-differentiation page explains why the format earns its place before the live demo. Preserving the findings, synthesis and solution beats retains the distinct value Ethan identified in each.
- Evidence boundary: the occasion framing is a working hypothesis. Kumar and Epley studied gratitude letters, and Givi and Galak studied anticipated reactions to non-occasion gifts; these are related mechanisms, not prevalence proof for the proposed occasion rule. The market gap is a working need-state, not validated market sizing or demand. The `72%` and `78%` on page 7 are temporary rehearsal placeholders, not verified primary findings, and Maya is illustrative rather than a validated persona. The four-part product combination is also a design proposition backed by current capabilities, not proof that Warm & Fuzzies is emotionally stronger than another format. The `sense of ownership` language means receiver control, not legal ownership; revisitability is browser-local rather than permanent storage; and the bearer link does not establish privacy. The `82%`, `71%`, decimal ratings, chart positions and circle sizes also remain temporary rehearsal material.

### Post-build judge review

| Judging lens | Effect | Evidence | Risk or unknown |
| --- | --- | --- | --- |
| Brief fit | strengthens conditionally | The deck now shows the team's starting assumption, the proposed occasion rule and the experience intended to redesign that rule. | The occasion rule and exact 2026 judging emphasis remain unverified. |
| Problem identification | strengthens | Visible reframing moves from the starting expression hypothesis to a direct event-versus-ordinary-day comparison, then asks the HMW; page 6 names the market-gap need-state before page 7 introduces a separate example. | No primary study yet establishes the prevalence of the occasion rule, market size, demand, cause or strongest audience context. |
| Solution approach | strengthens | Page 10 states four implemented qualities in plain language—letter personality, mixed-media dynamism, text-like access and receiver control—before the live flow proves the interaction. | Current capability does not prove that the experience reduces awkwardness or outperforms an existing channel. |
| Design innovation | strengthens | A single brace makes the synthesis explicit: four strengths from familiar forms become one firefly-branded digital gift rather than another feature list. | Without matched testing, judges may still interpret the result as a decorated digital letter. |
| Visual communication | strengthens | Page 3 stages the hypothesis, `show it`, event side, ordinary-day side and HMW in order; page 6 ends on a curved pale-blue market-gap band with only the limbo headline; page 7 uses three builds for Maya and the two placeholders; page 10 reveals four literal points and groups them with a brace into one firefly-branded digital gift; the repaired page-11 QR asset renders and machine-decodes to `/demo/receive` at delivery size. | The band, use-case figures, brace relationship and evidence boundaries must remain legible at projector distance, and the QR still requires a physical projected scan. |
| Presentation skills | strengthens | The deck reaches a concrete product explanation before the 100-second demo and now plans for 5:15, leaving 0:45 inside the working six-minute heat limit. | The team must rehearse the added market-gap-to-Maya handoff, the transition from differentiation to demo and verified team metadata on the cover. |
| Evidence quality | weak | The deck and notes label the occasion rule as a working hypothesis, the cited papers as related mechanisms rather than prevalence proof, page 6 as an unvalidated need-state, Maya as illustrative and all four visible percentages as rehearsal material. | Temporary findings and comparison values—including the new 72% and 78%—remain the largest judged-delivery credibility risk. |

### Required challenge review

| Challenge lens | Strongest challenge | Evidence-backed answer | Blocking unknown |
| --- | --- | --- | --- |
| Assumption and inversion | The ordinary-day gap may not be caused by a perceived need for an occasion. | Page 3 labels the occasion rule as a working hypothesis and makes the event-versus-ordinary-day contrast explicit enough to test. | Primary evidence showing whether the occasion prompt changes expression behaviour, for whom and in what context. |
| Behaviour and context | The audience may be too broad to share one barrier. | Page 6 keeps university students, long-distance family and close friends as working contexts; page 7 then uses one concrete moment in which Maya wants to thank someone for being so friendly during her first week at uni. | Which real relationship context most consistently experiences the barrier. |
| Medium necessity | A new digital product may not be necessary. | The solution can combine authored writing, photo, voice, video and song in one asynchronous object and open without a receiver account. | Whether that combination creates enough additional value to justify another product. |
| Alternatives | A voice note, letter, gift or non-digital ritual may work better. | Each remains an explicit comparison rather than a straw man. | Direct, content-matched evidence across the alternatives. |
| Human specificity | The Maya use case and adjacent percentages could be mistaken for a validated persona or participant evidence. | Page 7 labels the scenario illustrative and its 72% and 78% values as temporary rehearsal placeholders; Maya remains separate from the HMW and market-gap claim. | A research-derived audience pattern and validated use case based on completed interviews. |
| Inclusion and accessibility | QR access, media creation and device assumptions can exclude people. | The scan is optional, the receiver needs no account and the presenter screen remains the no-device route. | Assistive-technology, low-bandwidth and real-device accessibility testing. |
| Trust, safety and privacy | A direct link can be forwarded or mistaken for private delivery. | The pitch describes a direct link and the controls actually shown; it does not claim encryption, verified identity or service-level privacy. | Real expectations around forwarding, retention and wrong-recipient recovery. |
| Failure and recovery | A broken QR or event network would interrupt the strongest proof moment. | Fixed presenter and receiver routes, a readable URL and pre-opened presenter states support recovery. | Projector-distance decoding and a second-device event-network rehearsal. |
| Feasibility and demo truth | A polished capability synthesis can imply validated product superiority or ownership. | The slide and footer limit the claim to current capability: receiver control is not legal ownership, revisitability is browser-local rather than permanent storage, and direct-link access is not private delivery. | Cross-device reliability and evidence of emotional outcome. |
| Incentives and second-order effects | Repeated keepsakes could create obligation, performance pressure or oversharing. | The receiver controls when to open and whether to keep or remove; no reply is required. | Behaviour and pressure after repeated use. |
| Simplicity and retellability | Eleven slides plus a live demo may still feel dense. | Page 3 turns the reframe into five ordered beats on one slide; page 6 states the market gap; page 7 holds the use case; and page 10 groups four literal strengths with one brace, preserving a 5:15 chain. | Timed first-listener recall, especially across pages 6–8, and speaker handoffs. |
| Authorship and distinctiveness | Four capability points could become generic product marketing. | The hand-drawn brace and team firefly make the synthesis one recognisable digital gift rather than a generic comparison diagram. | Whether the brace-to-gift relationship remains clear when projected and narrated at speed. |

- Verdict: **Pass for rehearsal / Test first for judged delivery**.
- Work permitted by this verdict: rehearse and visually repair the eleven-slide baseline; replace temporary material only with completed evidence; add verified team metadata; run the named comparison and device checks.
- Blocking conditions: keep the occasion rule visibly framed as a working hypothesis and the cited papers as related mechanisms rather than prevalence proof; keep page 6 framed as a working need-state rather than validated market sizing or demand; replace or remove the 82%, 71%, 72%, 78%, every decimal rating, plotted position and frequency size before judged delivery; add the verified team number and member names; confirm the `/demo/receive` QR scans physically from the projected deck; rehearse the full 5:15 sequence and live handoff on a second device; keep Maya's illustrative and not-validated-persona status explicit.
- Weakest judging angle: **evidence quality**, because the four prominent percentages are still rehearsal values, the market gap is unvalidated and Maya's use case is not participant research.
- Main trade-off: separating the market gap from Maya prevents one illustrative case from standing in for demand, but adds twenty seconds and another handoff to the pitch.
- Next evidence that could change the verdict: one documented, content-matched sender-and-receiver comparison across an existing channel, Warm & Fuzzies and one non-digital control, followed by timed first-listener recall.
- Recommendation: use the eleven-slide sequence as the rehearsal baseline, then advance to judged delivery only after the evidence, metadata, QR and second-device conditions pass.
- Revisit when: completed primary findings, an evidenced audience pattern or a timed rehearsal requires a different sequence.

### Design-authenticity post-build review

- Communication job: show how the team's reasoning changed from a general expression gap to a working event-versus-ordinary-day rule, separate the hypothesised market gap from one illustrative case, then make why this format earns its place understandable without turning the pitch into a feature tour.
- Specific source anchors: the event-versus-ordinary-day split; the pale-blue band held between the Goldilocks rings; Cecelia's hand-drawn firefly and envelope; Gaegu and deep-ink-on-paper language; one hand-drawn brace; one firefly-branded digital gift; the receiver-controlled `keep, revisit or remove` ending.
- Rejected defaults: no demographic archetype card; no equal feature-card grid; no fake phone UI; no decorative diamonds or faux keepsake container; no second conclusion; no screenshot carousel, Venn or generic competitor table.
- Signature object: one hand-drawn brace gathers four literal strengths into the firefly-branded Warm & Fuzzies digital gift.

| Authenticity challenge | Post-build result | Remaining action |
| --- | --- | --- |
| First visual impression | Page 3 reads as a visible change in thinking: hypothesis, `show it`, event-side prompt, ordinary-day absence and HMW; page 6 reads as a curved market gap between two constraints with no redundant explanation; page 7 reads as one explicitly illustrative Maya case; page 10 reads as four literal strengths grouped into one digital gift. | Confirm those priorities survive the projector and speaker timing. |
| Logo-swap test | Pass: the occasion tension, Goldilocks gap, authored object and receiver-controlled ending depend on this project rather than a generic app. | Keep generic feature language out of page 10. |
| Team-voice test | Pass: Ethan directly split page 3 into the event and ordinary-day sides under the exact unwritten-rule headline, retained the findings/synthesis/solution beats and broad audience boundary, reduced page 6 to the single limbo headline, rewrote Maya as one direct illustrative sentence and replaced the Venn with four literal points grouped into one digital gift. | Rehearse the visible labels exactly as the team will say them. |
| Subtraction pass | Process diary, demographic-archetype metadata, feature cards, fake object art, repeated conclusion and a separate QR ending remain removed. | Do not reintroduce explanatory microcopy during final polish. |
| Hierarchy | Each of the first three clicks adds one literal strength; the fourth adds the ownership point, brace and firefly-branded digital gift as the visual conclusion. | Check the brace, gift mark and footer at room distance. |
| Delivery-size QA | The earlier page-3 and ring-based product-slide inspections no longer certify the current split, market-gap, Maya and brace layouts. The page-11 QR still renders correctly at 1600 × 900 and machine-decodes to `https://warm-and-fuzzies.vercel.app/demo/receive`. | Reinspect the updated pages 3, 6, 7 and 10 at 1600 × 900, then scan the QR physically from the projected deck before calling delivery ready. |

- Strongest remaining AI tell: the four points could read as generic marketing if the presenter does not map each one back to the earlier channel research and keep the capability boundary explicit.
- Material human-directed change: Ethan required the problem and reframe to merge, split the new page 3 into event and ordinary-day sides under `The unwritten rule: appreciation needs an occasion.`, protected the distinct value of the findings and synthesis slides, rejected a narrowed market, separated and copy-trimmed the pale-blue market gap from Maya's illustrative three-build use case and replaced the Venn with four literal strengths grouped by a brace into one firefly-branded digital gift.
- Authenticity verdict: **Pass for rehearsal; judged delivery remains Test first under the evidence gate**.
...
## 2026-09-06 - Simplify the appendix into the 2025 winner's editorial research-slide format

- Status: current presentation-support direction, selected directly by Ethan after reviewing the first 15-page appendix against the 2025 winner's reference appendix.
- Decision: replace the governance-heavy bespoke 15-page evidence dossier with a 13-page appendix that follows the winner's repeated editorial research-slide grammar: large page title, sparse readable source material or raw artifact, and direct blank templates where research has not been completed. The sequence is appendix divider; brief; two secondary-research pages; research questions and problem statement; blank interview structure, interview findings, survey findings I, survey findings II and task-based findings; process artifacts; current visual direction/prototype; then references, credits and disclosure blanks. Preserve the separate route, A1–A13 deep links, keyboard/index/fullscreen/print behaviour and the single return to live slide 11.
- Reason: the first appendix was accurate but too dense and visually unlike the reference. The winner's repeated format lets a judge scan the evidence quickly and makes missing work obvious without disguising it as a finished governance dashboard.
- Evidence boundary: keep the Warm & Fuzzies identity without copying the winner's wording, palette or illustrations. Do not invent interview, survey or task-based results, claims, percentages, team information or disclosure details; leave each incomplete record visibly blank for the team to fill. Secondary sources remain adjacent-mechanism support, while prototype captures remain evidence only of implemented behaviour.
- Consequence: the main eleven-slide pitch remains unchanged; the appendix becomes a clearer evidence-and-process companion that is easier to complete manually before submission.
